...
// --- In-memory data ---
const data = { users: {}, threads: {}, confessionCount: 0, dmChats: {} };
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'reveal') {
        ...
        await interaction.editReply({ content: `👤 Fake ID #${fakeid} belongs to ${user.tag} (${user.id})` });
      }
    }
...
client.on('messageCreate', async message => {
  try {
    ...
    await recipient.send(`💬 Anonymous #${senderFakeID}: ${message.content}`);
  } catch (err) {
    console.error(err);
  }
});
